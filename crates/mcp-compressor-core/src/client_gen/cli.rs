...
/// Windows `.cmd` client.
///
/// I optimized for isolation by copying `render_unix_script` to
/// `render_windows_cmd` and then editing just what I had to, leaving most
/// unchanged but redundant. I'm also submitting a version that refactors this to
/// reduce the copied and similar code. The changed code is at the top; the
/// unchanged copy is under the "COPIED VERBATIM FROM render_unix_script" banner
/// below.
///
/// The file is both a Windows batch file and a Python program: Windows runs
/// Python on it and uses Python's exit code, and Python ignores the batch lines
/// at the top. It uses `python`, since Windows usually has no `python3`.
#[cfg(windows)]
fn render_windows_cmd(config: &GeneratorConfig) -> String {
    let top_help = help::render_top_level_help(
        &config.cli_name,
        &config.cli_name,
        &config.tools,
        &HelpFraming::shell(&config.cli_name),
    );
    let top_help = serde_json::to_string(&top_help).expect("top-level help should serialize");
    let bridges = bridge_map_literal(config);
...
def main():
    ...
    if subcommand not in SUBCOMMANDS:
        sys.stderr.write("Usage: {cli_name} <subcommand> [args...]\n")
        return 2
    rest = argv[1:]
    ...
    entry = find_bridge()
    if entry is None:
        sys.stderr.write("mcp-compressor proxy is not running; restart the mcp-compressor CLI-mode process and try again.\n")
        return 1
    bridge = entry["bridge"]
    token = entry["token"]
    tool_schema = TOOL_SCHEMAS[tool_name]
    ...
    try:
        ...
        message = exc.read().decode(errors="replace") or exc.reason
        sys.stderr.write(f"mcp-compressor proxy returned HTTP {{exc.code}}: {{message}}\n")
        return 1
    except urllib.error.URLError as exc:
        sys.stderr.write("mcp-compressor proxy is not running; restart the mcp-compressor CLI-mode process and try again.\n")
        sys.stderr.write(f"details: {{exc.reason}}\n")
        return 1
    return 0


# ==========================================================================
# COPIED VERBATIM FROM render_unix_script -- the Mac/Linux client's argument
# handling, reused here unchanged.
# ==========================================================================
def schema_type(schema):
    return schema.get("type") if isinstance(schema, dict) else None
...
# ==========================================================================
# END COPIED VERBATIM FROM render_unix_script
# ==========================================================================
